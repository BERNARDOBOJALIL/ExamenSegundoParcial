...
import { FaCheckCircle, FaClock, FaCreditCard, FaUtensils } from 'react-icons/fa';
...
const OrderStatusAdmin = () => {
      ...
      try {
        unsubscribe = getOrders(
          ...
          (fetchedOrders) => {
            ...
            const filteredOrders = fetchedOrders.filter(order => order.state !== 'Recibido');
  ...
  const handleSetReady = async (orderId) => {
    try {
      ...
      console.log(`Orden ${orderId} marcada como Listo`);
    } catch (error) {
  ...
  return (
    <div className="mt-10 mb-10 px-4">
      <h2 className="text-3xl font-bold mb-5 flex items-center gap-2 mt-20">Órdenes activas</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {orders.map((order) => (
          <div
            key={order.id}
            className={`p-6 shadow-lg rounded-lg border-4 flex flex-col justify-between h-full ${getStatusColor(order.state)}`}
          >
            <div>
              <div className="flex items-center">
                <p className="font-semibold text-lg">ID de Orden: <span className="font-normal text-blue-600">{order.id}</span></p>
              </div>
              <p className="text-sm text-gray-500 mt-1">Fecha: {new Date(order.timestamp.seconds * 1000).toLocaleString()}</p>
              <p className="text-sm text-gray-500">Cliente: {order.client || 'No especificado'}</p>
              <div className="flex items-center">
                <p className="text-sm text-gray-500 mr-2">Estado:</p>
                {order.state === 'En preparación' && <FaClock className="text-yellow-600" />}
                {order.state === 'Listo' && <FaCheckCircle className="text-green-600" />}
