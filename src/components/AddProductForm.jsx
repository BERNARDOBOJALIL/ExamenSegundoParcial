...
const AddProductForm = () => {
  ...
  useEffect(() => {
    const unsubscribe = getMenu((data) => setMenuItems(data));
  ...
  const handleSaveProduct = async (action) => {
    setLoading(true);
    try {
      let imageUrl = selectedProduct
        ? menuItems.find((item) => item.id === selectedProduct)?.image
        : "";
  ...
  return (
    <div className="mt-16 min-h-screen bg-yellow-100 p-5">
      <h2 className="text-3xl font-bold mb-5 flex items-center justify-center gap-2">Administración de Productos</h2>
          ...
          <TabsContent value="register">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                />
                {imageFile && (
                  <p className="text-gray-600 truncate">{imageFile.name}</p>
                )}
              </div>
              <button
                type="submit"
